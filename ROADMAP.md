...
> Próxima sprint oficial: **MOOD-IMPACT-2 — Lag & Hypothesis Lab**.
...
| 3 | **MOOD-IMPACT-2 — EM ANDAMENTO** | Lag & Hypothesis Lab: métrica → humor com lags 0-3d, qualidade do sinal e baseline pessoal | Transforma correlação em hipótese testável, não em afirmação clínica. |
...
Status 2026-04-30: primeira fatia concluída. `MoodLagHypothesisLab` foi adicionado à aba Insights via `CorrelationHeatmap`; ele usa `DailySnapshot[]` existente, permite selecionar sono total, HRV, FC repouso, passos, luz do dia ou doses logadas, e mostra lags `0d` a `3d` com `n`, qualidade do sinal, Pearson `r`, baseline acima/abaixo da média pessoal da métrica e alerta de sampling bias. Teste focal criado em `frontend/tests/mood-lag-hypothesis.test.ts`.

Escopo:

- [x] Selector de métrica contra humor.
- [x] Lags `0d`, `1d`, `2d`, `3d`.
- [x] `n` de pares, qualidade do sinal e aviso de sampling bias.
- [x] Baseline pessoal: mood quando métrica está acima/abaixo da própria média.
- [ ] Variância Lamictal vs SD rolling 7d do humor: estacionada para a próxima fatia farmacológica, porque exigiria reabrir a camada PK diária enquanto a entrega útil já ficou acessível.
...
2. Continuar **MOOD-IMPACT-2** em fatia pequena: Lamictal variance só se couber sem reabrir a arquitetura PK/humor.
3. Preservar o Medication Action Center, `MoodDriverBoard` e `MoodLagHypothesisLab` já implementados.
4. Validar com `npx tsc --noEmit`, `npm run build` e `git diff --check`.
5. Atualizar docs apenas com status real do que foi concluído.

Depois:

- Quando MOOD-IMPACT-2 fechar, MOOD-IMPACT-3 aprofunda luz, sono, HRV/RHR, respiração, SpO2 e temperatura em perguntas clínicas separadas.
