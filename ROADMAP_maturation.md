...
> Estado: Sprints M1 + M2 concluídas; próxima execução: Sprint M3
...
## Sprint M3 — Sinais Vitais: Wrist Temp Deviation + remover badge "Hipotermia"
...
## KICKOFF — Sprint M3 (próxima sessão fresh)
...
**Objetivo:**
1. Remover bug semântico: badge "Hipotermia" pra 35.9°C de wrist
   temperature (Apple Watch mede desvio do pulso, NÃO core temp — chamar
   isso de "hipotermia" é clinicamente incorreto).
2. Substituir Temp Pulso linear absoluto por Wrist Temp Deviation
   (delta da baseline pessoal, eixo Y centralizado em 0).
3. Manter FR + adicionar FR variability rolling SD 7d como derivação
   complementar.

**Arquivos-alvo:**
- /root/RooCode/frontend/src/components/charts/vital-signs-timeline.tsx
- /root/RooCode/frontend/src/utils/health-policies.ts (atualizar
  getPulseTempCategory ou substituir por getWristTempDeviationCategory)
- NOVO: /root/RooCode/frontend/src/utils/personal-baselines.ts (utility
  reusada em M4 e M5 — `computeRollingBaseline(values, minPoints, windowDays)`)

**Hipótese pré-registrada:**
"Desvio de wrist temp >+0.5°C sustentado por 2-3 noites pode preceder
doença infecciosa subclínica. FR variability ↑ (SD rolling >2.5 rpm)
pode preceder estresse autonômico/ansiedade ou início de quadro
respiratório."

**Decisões metodológicas:**
- Wrist temp baseline: média das últimas 30 medições válidas (excluir
  interpolated/forecasted — regra interim conservadora da M6).
- Mínimo de 14 medições reais pra calcular baseline (DataReadinessGate).
- Delta = `temp_today - personal_baseline` em °C.
- Bandas: [-1.0, -0.3] queda anormal · [-0.3, +0.3] normal ·
  [+0.3, +0.5] elevado · [+0.5, +1.5] alerta.
- Badge atualizado: "Desvio: +0.4°C ↑" / "Normal · 0°C" / "Alerta · +0.7°C".
- FR variability: rolling SD 7d, mostrado como segunda linha tracejada
  no painel de FR.

**Plano:**
1. Read vital-signs-timeline.tsx — entender estrutura.
2. Read health-policies.ts — ver getPulseTempCategory atual (linha ~161).
3. Criar personal-baselines.ts com `computeRollingBaseline(values,
   minPoints, windowDays)` — utility reusada em M4/M5 (escopo isolado).
4. Edit vital-signs-timeline.tsx — substituir absoluto por delta,
   atualizar badge, adicionar FR SD 7d como linha tracejada.
5. Adicionar test em tests/personal-baselines.test.ts (sanity da
   rolling baseline + edge cases).
6. Validação: tsc + lint + test:unit + build + manual UI.
7. Commit.

**Validação:**
- Badge "Hipotermia" não aparece mais.
- Chart mostra delta da baseline pessoal (eixo Y centralizado em 0).
- FR mantém + nova linha tracejada de SD 7d.
- Tooltip esclarece "wrist temp deviation".
- DataReadinessGate ativa se <14 dias de baseline.

**Não tocar:** outros charts da aba Sinais Vitais. PersonalBaselines.ts
deve ser genérica o suficiente pra ser reusada em M4 (Recovery Score)
e M5 (ABI) sem refactor.

**Pós-Sprint Protocol obrigatório** (ver `/root/.claude/rules/sprint-system.md` regra 7):
- Marcar M3 como concluída no ROADMAP_maturation.md (status + commit hash).
- Atualizar tabela executiva (✅ + hash).
- Adicionar bloco "Status / Resultado" no topo da seção M3.
- Reescrever este KICKOFF apontando pra Sprint M4 (Recovery Score
  composto na aba Panorama). M4 vai reusar `personal-baselines.ts`
  criado nesta sprint — sinalizar isso no KICKOFF da M4.
- Atualizar CLAUDE.md raiz: adicionar M3 nas concluídas + linha no
  Status local sobre o que mudou.
