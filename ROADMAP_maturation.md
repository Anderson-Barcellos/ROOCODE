...
> Estado: Sprint M1 concluída (commits `b0622ff` + `6b1bc07`); próxima execução: Sprint M2
...
| M2 | Atividade | VO2 Máx via Uth-Sørensen (substitui empty) | Baixo | 1 sessão |
...
## Sprint M2 — VO2 Máx via Uth-Sørensen
...
## KICKOFF — Sprint M2 (próxima sessão fresh)

```
Olá Claude! Sou o Anders. Estamos retomando o RooCode (`/root/RooCode`).
Sessão fresh — siga sprint-system.md e o protocolo de fresh start do AGENTS.md.

# Sprint M2 — VO2 Máx via Uth-Sørensen

**Objetivo:** Substituir empty state do `Vo2MaxChart` por estimação de VO2
via fórmula Uth-Sørensen (`VO2max ≈ 15 × HRmax/RHR`), preenchendo o chart
com derivação calculada a partir de RHR diário.

**Arquivos-alvo:**
- /root/RooCode/frontend/src/components/charts/vo2-max-chart.tsx
- /root/RooCode/frontend/src/utils/health-policies.ts (helper novo: estimateVo2MaxUthSorensen)

**Hipótese pré-registrada:**
"VO2 Máx estimado deve cair quando RHR sobe (overtraining ou fadiga) e
subir com treinamento aeróbico crônico. HRmax fixo (220-idade) não muda;
só RHR varia diariamente. Esperar baseline ~35-50 ml/(kg·min) pra Anders."

**Decisões metodológicas:**
- HRmax: `220 - 38` (idade Anders, hardcoded com TODO pra config user-level)
- RHR: `restingHeartRate` por dia (não SMA — derivação diária)
- Cálculo: `vo2Est = 15 * (HRmax / RHR)` por dia onde RHR != null
- Header: "VO2 Máx estimado (Uth-Sørensen)"
- Bands: `VO2_BANDS_MALE_35_44` já existe em health-policies.ts — reusar
- Caveat clínico no <details>: "Estimativa por proxy Uth-Sørensen.
  Acurácia ~85% vs CPET. Não substitui medida direta em laboratório."

**Plano:**
1. Read vo2-max-chart.tsx — entender estrutura atual e empty state.
2. Read health-policies.ts — adicionar helper
   `estimateVo2MaxUthSorensen(rhr: number, hrMax: number): number`.
3. Edit vo2-max-chart.tsx — substituir lógica que produz empty state
   por estimativa quando RHR válido; manter empty quando RHR null.
4. Atualizar header + tooltip + caveat clínico.
5. Validação: tsc + lint + test:unit + build + manual UI.
6. Commit.

**Validação:**
- Vo2MaxChart renderiza linha com VO2 estimado em todas datas com RHR válido.
- SMA-7d aparece (linha sólida).
- Bandas comparativas cobrem 25-55 ml/(kg·min).
- Header explícito "estimado".
- Range plausível (~38-48 esperado pro Anders dada RHR ~55-65).

**Não tocar:** outros charts da aba Coração (HRRangeChart, CardioRecoveryChart,
HrvAnalysis, HeartRateBands). HrvAnalysis e HeartRateBands serão tratados
em sprints futuras (M5).

**Pós-sprint protocol obrigatório** (ver `/root/.claude/rules/sprint-system.md` regra 7):
- Marcar M2 como concluída no ROADMAP_maturation.md (status + commit hash).
- Atualizar tabela executiva (✅ + hash).
- Adicionar bloco "Status / Resultado" no topo da seção M2.
- Reescrever este KICKOFF apontando pra próxima sprint (M3 ou brainstorm M6).
- Atualizar CLAUDE.md raiz se necessário (deve só linkar pro ROADMAP).
