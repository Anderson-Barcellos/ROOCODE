...
> Estado: planejado, aguardando execução em sessões fresh
...
|---|---|---|---|---|
| M1 | Farmaco | Debug do PKStandardDoseComparison | Baixo | 1 sessão |
...
## Sprint M1 — Farmaco debug do PKStandardDoseComparison
...
## KICKOFF — Sprint M1 (próxima sessão fresh)
...
**Objetivo:** Fix do chart "Curvas comparativas das 3 medicações" 
(`frontend/src/components/charts/pk-standard-dose-comparison.tsx`) que 
mostra eixo Y até 420% mas só renderiza 1 ponto vertical no extremo 
direito do eixo X. Tu disse pra NÃO remover, apenas ajustar.

**Arquivos-alvo:**
- /root/RooCode/frontend/src/components/charts/pk-standard-dose-comparison.tsx
- /root/RooCode/Farma/medDataBase.json (verificação)

**Background técnico:**
- domain={[0, 160]} no YAxis (linha ~245), mas screenshot mostrou 
  valores até 420%. Recharts pode estar auto-expandindo + truncando.
- Cálculo de %ref linha 179: (concentration / denominator) * 100
  onde denominator = therapeuticRange?.max
- Possível mismatch de unidades (ng/mL vs mcg/mL)

**Plano:**
1. Read medDataBase.json — verificar therapeuticRange.max das 3 drogas
   (Lamictal, Lexapro, Venvanse) e suas unidades.
2. Read pk-standard-dose-comparison.tsx — focar linhas 154-189 (cálculo) 
   e 244-251 (YAxis).
3. Rodar `cd /root/RooCode/frontend && npm run dev`, abrir aba Farmaco, 
   DevTools — capturar model.chartData real.
4. Diagnosticar entre 4 hipóteses:
   a) Domain Y truncando
   b) therapeuticRange.max errado/zerado
   c) Unidade mismatch (ng/mL vs mcg/mL)
   d) Timestamps colapsados pelo scale="time"
5. Fix dirigido + commit.

**Validação:**
- 3 curvas distintas (Lamictal, Lexapro, Venvanse)
- Cada curva oscila ao longo do tempo (6 dias visíveis)
- Y axis range plausível (~0-200%)
- Tooltip funcional

**Não tocar:** outros charts da aba Farmaco.
